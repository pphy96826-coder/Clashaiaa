...
控制台提供模型切换、暂停/恢复、接管当前对局、连续对战、自动表情和局面预测开关。自动表情使用校准后的固定触摸点，不启用图像识别；连续对战会处理结算页的两个 `OK` 按钮，再确认回到大厅后启动下一局。

局面预测是可选的离线镜像：它从当前已确认状态建立快照，把已确认的己方动作按原生部署时序重放，并向前外推一段时间后再交给模型。它不会猜测未知的对手动作；对手完整回放不可用、牌组不兼容、镜像漂移或引擎返回非状态响应时，预测会自动失效，模型回到实时原生观测。

### 3.2 离线模拟的工作原理

离线引擎运行在独立的 Android 实例中，通过本机端口提供战斗状态服务。在线游戏仍由 MuMu 和服务器推进，离线实例只作为“镜像计算器”，不会替代在线实例，也不会向在线实例写入动作。

每次镜像更新大致经过以下步骤：

1. 从在线 probe 取得一份完整且通过身份、tick、实体一致性校验的 `ObservationV1`。
2. 在镜像中从当前边界建立或恢复快照；快照包含双方已知实体、塔、圣水、手牌、效果和当前 tick。
3. 将已经在在线局面中确认的己方动作加入镜像，并按真实的触摸、排队、部署动画和原生 consume 时序重放，而不是在点击瞬间把单位直接放到战场上。
4. 让离线引擎从该边界向前推进预测窗口，默认目标是覆盖端到端延迟和部署延迟；预测结果带有 `simulation_used`、起止 tick、镜像版本和漂移状态。
5. 只有结果完整、镜像没有漂移、并且仍属于同一 battle identity 时，才把预测状态交给模型；否则使用最新实时观测。

镜像不是“预知未来”。己方动作在确认后可以确定地重放；对手在预测窗口内尚未发生的动作无法确定。当前实现不会凭空猜对手牌，也不会把“当前敌方状态”和“未来己方状态”拼成一帧，因此缺少完整对手回放时会主动停用镜像。真实对手动作一旦被 probe 观察到，后续同步会以在线状态为准，必要时丢弃旧快照并从新的确认边界重建。

当前实现以“确认边界 + 重放 + 漂移检测”为主，而不是完整的任意时间回滚。它可以保存/恢复镜像快照并做分支外推，但在线状态已经越过的时间不会被无条件改写；如果实体、tick、资源或身份不一致，镜像必须失效，不能继续累积错误。

### 3.3 优点、限制与改进方向

优点：离线推进速度远高于在线 tick，能在模型决策前补偿观测、推理、触摸和部署造成的时间差；可以把刚确认的己方牌按正确部署时序加入，减少“模型看到的牌还没落地”造成的重复解牌；快照/分支机制也便于做只读回归，不触碰在线游戏。

限制：离线引擎与在线版本、probe 数据结构和牌组形态必须匹配；未知对手动作会造成预测分叉；英雄技能、觉醒、攻击目标切换、射程停步、伤害/死亡、投射物和特殊效果若未被引擎完整支持，预测可能比实时观测更差；独立 Android 实例还会增加维护成本，服务 ready 也不代表每一帧预测有效。

改进方向按优先级为：

- 把在线确认事件做成带 tick/sequence 的事件日志，支持从最近确认边界快速重建，而不是频繁全量重置。
- 增加 checkpoint ring buffer 和真正的分支预测：保留多个候选未来，观察到对手新动作后只淘汰不一致分支。
- 对单位加入攻击状态、射程、目标选择、伤害、死亡、部署和投射物规则，并让镜像输出置信度；低置信度时自动回退实时状态。
- 统一在线与离线引擎的卡牌、坐标、形态和 tick 合约，建立 500ms/1s 窗口的逐 tick 差分测试。
- 单独统计镜像更新耗时、模型等待耗时、触摸耗时和漂移率，避免把离线引擎很快误认为端到端延迟已经很低。
